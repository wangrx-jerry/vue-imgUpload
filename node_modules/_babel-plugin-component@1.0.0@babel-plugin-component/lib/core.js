      ...
      if (!selectedMethods[methodName]) {
        ...
        if (styleLibraryName) {
          ...
          if (libraryObjs[methodName]) {
            ...
            if (styleRoot) {
              path = "".concat(cachePath[libraryName]).concat(styleRoot, ".css");
            } else {
              path = "".concat(cachePath[libraryName]).concat(_root || '/index', ".css");
            }
            ...
            if (cache[libraryName] !== 1) {
              ...
              if (modulePathTpl) {
                ...
                path = "".concat(cachePath[libraryName], "/").concat(parsedMethodName, ".css");
              }

              if (mixin && !isExist(path)) {
                path = style === true ? "".concat(_path, "/style.css") : "".concat(_path, "/").concat(style);
              }

              if (isBaseStyle) {
                addSideEffect(file.path, "".concat(cachePath[libraryName], "/base.css"));
              }
          ...
          if (style === true) {
            addSideEffect(file.path, "".concat(path, "/style.css"));
          } else if (style) {
